...
*}
{capture name=path}{l s='Billing Detail'}{/capture}
<div class="box billing-detail col-lg-7 col-sm-7 col-xs-12">
{*	<div class="billingAddressLine">
		<p>
			<span>{l s='Returning customer?'}</span><a href="#">{l s='Click here to login'}</a>
		</p>
		<p>
			<span>{l s='Have a coupon?'}</span><a href="#">{l s='Click here to enter your code'}</a>
		</p>
	</div>*}
	<h1 class="page-subheading">{l s='Billing Detail'}</h1>
	<p class="info-title">
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			...
			{if $field_name eq 'address1'}
				<div class="required form-group">
					...
					<textarea class="is_required validate form-control" data-validate="{$address_validation.$field_name.validate}" id="address1" name="address1" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{else}{if isset($address->address1)}{$address->address1|escape:'html':'UTF-8'}{/if}{/if}">{if isset($smarty.post.address1)}{$smarty.post.address1}{else}{if isset($address->address1)}{$address->address1|escape:'html':'UTF-8'}{/if}{/if}</textarea>
				</div>
			{/if}
			...
			{if $field_name eq 'phone_mobile'}
				...
				<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group phone_mobile">
		...
		<p class="submit2" style="margin-bottom: 20px;">
		...
		<a href="{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}" style="display:block; text-align: center;">
			<span><i class="icon-chevron-left hidden"></i> {l s='Back to addresses'}</span>
		</a>
</div>
{strip}
