...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{else}
					<a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default ContinueShop"><span><i class="icon-chevron-left"></i>{l s='Continue Shopping'}</span></a>
				{/if}
		        <button type="submit" name="processAddress" class="button btn btn-default button-medium"><span>{l s='Order Now'}<i class="icon-chevron-right right"></i></span></button>
			</p>
		</form>
{else}
	</div>
