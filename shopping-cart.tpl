...
<div class="header-my-cart">
	<span class="heading-counter">{l s='You Have'}
		<span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='Items'}{/if}</span>&nbsp;{l s='In Your Cart'}
	</span>
</div>
{/if}
...
{else}
		...
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<thead>
				<tr>
					...
					<th class="cart_unit item text-center">{l s='Price'}</th>
			...
			<tfoot>
				...
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_price">
							<td rowspan="{$rowspan_total}" colspan="3" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											<h4>{l s='Have A Promotional Code?'}</h4>
											<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
											<input type="hidden" name="submitDiscount" />
											<button type="submit" name="submitAddDiscount" class="button btn btn-default button-small"><span>{l s='Apply'}<i aria-hidden="true" class="fa fa-long-arrow-right"></i></span></button>
										</fieldset>
									</form>
									{if $displayVouchers}
							...
							<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Subtotal'}{/if}</td>
					...
					{else}
						<tr class="cart_total_price">
							...
							<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Subtotal'}{/if}</td>
				...
				{else}
					<tr class="cart_total_price">
								...
								<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
									<fieldset>
										...
										<button type="submit" name="submitAddDiscount" class="button btn btn-default button-small">
											<span>{l s='Apply'}<i aria-hidden="true" class="fa fa-long-arrow-right"></i></span>
										</button>
									</fieldset>
								</form>
								{if $displayVouchers}
						...
						<td colspan="{$col_span_subtotal}" class="text-right">{l s='Subtotal'}</td>
						<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
					</tr>
				{/if}
					...
					{/if}
						<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
							...
							<td class="cart_discount_delete">1</td>
                                                        <td class="cart_discount_price text-centered">
								<span class="price-discount">
	...
	<p class="cart_navigation clearfix">
		{if !$opc}
			<a  href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}" class="button btn btn-default standard-checkout button-medium" title="{l s='Order Now'}">
				<span>{l s='Order Now'}<i class="icon-chevron-right right"></i></span>
			</a>
		{/if}
		<a href="https://www.mon-savonnier.fr" style="display: block; text-align: center; margin-top: 15px;" title="{l s='Continue shopping'}">
