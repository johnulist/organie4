...
{if !$opc}
	...
	<div id="carrier_area">
		<h1 class="page-heading hidden">{l s='Shipping'}</h1>
...
{/if}
<div class="order_carrier_content" style = "margin-top: 20px">
	...
	{else}
				...
				{if isset($delivery_option_list)}
					{foreach $delivery_option_list as $id_address => $option_list}
						...
						<div class="delivery_options">
						    {assign var="alreadyChecked" value=false}
							{foreach $option_list as $key => $option}
							    {assign var="display" value=true}
								{foreach $option.carrier_list as $carrier}
									{if $carrier.instance->id == 39}
									    {if ! $address_collection[$id_address]->city|lower|in_array:['lyon','villeurbanne']}
                                        ...
                                        <table class="resume table {if !$option.unique_carrier} hide{/if}">
                                            <tr>
                                                <td class="delivery_option_radio">
                                                    <input id="delivery_option_{$id_address|intval}_{$option@index}" class="delivery_option_radio" type="radio" name="delivery_option[{$id_address|intval}]" data-key="{$key}" data-id_address="{$id_address|intval}" value="{$key}"{if !$alreadyChecked} checked="checked"{/if} />
                                                    {assign var="alreadyChecked" value=true}
                                                </td>
		...
		{if !$opc}
				<p class="cart_navigation clearfix">
					...
					{if !$is_guest}
						{if $back}
							<a href="https://www.mon-savonnier.fr" title="{l s='Previous'}" style="display:block; margin:auto;">
								<span><i class="icon-chevron-left"></i>
								{l s='Continue shopping'}</span>
							</a>
						{else}
							<a href="https://www.mon-savonnier.fr" title="{l s='Previous'}" style="display: block; text-align: center; margin-top: 20px;">
								<span><i class="icon-chevron-left"></i>
								{l s='Continue shopping'}</span>
							</a>
						{/if}
					{else}
						<a href="https://www.mon-savonnier.fr" title="{l s='Previous'}" class="button-exclusive btn btn-default ContinueShop">
							<span><i class="icon-chevron-left"></i>
							{l s='Continue shopping'}</span>
						</a>
					{/if}
