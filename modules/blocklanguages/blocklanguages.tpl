...
<div class="dorLanguagesBlock">
	<h3><span class="txt-code-label">{l s='Language' mod='blocklanguages'}:</span>
		{foreach from=$languages key=k item=language name="languages"}
			{if $language.iso_code == $lang_iso}
				<span class="txt-iso-code"><img alt="{$language.iso_code}" height="11" src="{$img_lang_dir}{$language.id_lang}.jpg" width="16" /><span class="code-iso">{$language.iso_code|regex_replace:"/\s\(.*\)$/":""}</span><span class="code-name">{$language.name|regex_replace:"/\s\(.*\)$/":""}</span></span>
			{/if}
		{/foreach}
	</h3>
		<div id="languages-block-top" class="languages-block">
			...
			<ul id="first-languages" class="languages-block_ul">
				{foreach from=$languages key=k item=language name="languages"}
					...
					<img alt="{$language.iso_code}" height="11" src="{$img_lang_dir}{$language.id_lang}.jpg" width="16" />
					{if $language.iso_code != $lang_iso}
