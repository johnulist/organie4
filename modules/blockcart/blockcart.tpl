...
{/if}
	<div class="shopping_cart{if !$products} dor_noproduct{/if}">
			...
			<div class="cart_block block exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						<h3 class="title-block-cart">{l s='Your Cart' mod='blockcart'} (<span id="cartCount">{count($products)}</span>)</h3>
						{if $products}
							<dl class="products scroll-div">
								{foreach from=$products item='product' name='myLoop'}
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										...
										<div class="cart-info">
											<div class="cart-info-block-left col-lg-6 col-xs-12">
												<div class="product-name">
													<a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:40:'...'|escape:'html':'UTF-8'} ({$product.cart_quantity})</a>
												</div>
											...
											<div class="cart-info-block-right col-lg-6 col-xs-12">
												<span class="price">
						...
						<p class="cart_block_no_products{if $products} unvisible{/if}">
							{l s='You have no items in your shopping cart.' mod='blockcart'}
						</p>
						...
						<p class="cart-buttons">
							<a id="button_order_cart" class="btn btn-default button button-small button_order_cart" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
									{l s='Go to cart' mod='blockcart'}<i class="icon-chevron-right right"></i>
								</span>
							</a>
							<a id="button_order_checkout" class="btn btn-default button button-small button_order_cart" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}?step=1" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
		...
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container" style="margin-top:20px">
						<a href="https://www.mon-savonnier.fr" title="{l s='Continue shopping' mod='blockcart'}">
