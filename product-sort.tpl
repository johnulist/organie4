...
<ul class="display hidden-xs">
    ...
    <li id="grid"><a rel="nofollow" href="#" data-toggle="tooltip" data-placement="top" title="{l s='Grid'}"><i class="fa fa-th"></i></a></li>
    <li id="list"><a rel="nofollow" href="#" data-toggle="tooltip" data-placement="top" title="{l s='List'}"><i class="fa fa-list"></i></a></li>
</ul>
...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1">
		<label for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="hidden">{l s='Sort by'}</label>
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort selectpicker">
			<option value="{if $page_name != 'best-sales'}{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}{/if}"{if !in_array($orderby, array('price', 'name', 'quantity', 'reference')) && $orderby eq $orderbydefault} selected="selected"{/if}>{l s='Default Shorting'}</option>
			{if !$PS_CATALOG_MODE}
